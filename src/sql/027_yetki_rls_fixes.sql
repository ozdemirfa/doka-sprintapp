...
-- ── kategoriler ───────────────────────────────────────────────
-- 003'te: kategoriler_insert_fatih, _update_fatih, _delete_fatih

DROP POLICY IF EXISTS "kategoriler_insert_fatih" ON kategoriler;
CREATE POLICY "kategoriler_insert_fatih"
  ON kategoriler FOR INSERT TO authenticated
  WITH CHECK (
    (SELECT rol_kodu FROM personel WHERE auth_id = auth.uid()) = 'yönetici'
    OR current_user_has_permission('kategoriler', 'yazma')
  );

DROP POLICY IF EXISTS "kategoriler_update_fatih" ON kategoriler;
CREATE POLICY "kategoriler_update_fatih"
  ON kategoriler FOR UPDATE TO authenticated
  USING (
    (SELECT rol_kodu FROM personel WHERE auth_id = auth.uid()) = 'yönetici'
    OR current_user_has_permission('kategoriler', 'guncelleme')
  )
  WITH CHECK (
    (SELECT rol_kodu FROM personel WHERE auth_id = auth.uid()) = 'yönetici'
    OR current_user_has_permission('kategoriler', 'guncelleme')
...
DROP POLICY IF EXISTS "kategoriler_delete_fatih" ON kategoriler;
CREATE POLICY "kategoriler_delete_fatih"
  ON kategoriler FOR DELETE TO authenticated
  USING (
    (SELECT rol_kodu FROM personel WHERE auth_id = auth.uid()) = 'yönetici'
    OR current_user_has_permission('kategoriler', 'silme')
  );
